...
Arena::Arena(int l, int w, Prey* prey, Predator* predator) : prey(prey), predator(predator) {
    ...
    // проверка размера арены
    if ((length > 99) || (length < 1) || (width > 99) || (width < 1)) {
        cout << "Введите правельные размеры арены (от 1 до 99)  " << endl;
        exit(0);
    }

    // проверка нахождения в арене prey
    if ((length < prey->getLocation().getX()) || (width < prey->getLocation().getY())) {
        cout << "Жертва находится за пределами арены  " << endl;
        exit(0);
    }

    // проверка нахождения в арене predator
    if ((length < predator->getLocation().getX()) || (width < predator->getLocation().getY())) {
        cout << "Хищник находится за пределами арены  " << endl;
        exit(0);
    }
...
// Были указаны (width, length) вместо (view_width,view_length), а также изменили 1 на 3
// исправив это, программа заработала корректно
