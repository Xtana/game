...
TEST(TestPoint2D, TestСonclusion) {
	...
	for (int i = 0; i < x_size - 1; i++)
	{
		for (int j = 0; j < y_size - 1; j++) {
			...
			testing::internal::CaptureStdout();                                                // Начинает захват из области stdout
			std::cout << point1;
			std::string output = testing::internal::GetCapturedStdout();					   // Заканчивает захват из области stdout и передает его
			ASSERT_EQ(output, s2);
